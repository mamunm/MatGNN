...
def get_atom_graph_data(
    ase_db_loc: str, params: AtomGraphParameters, dtype: torch.dtype
) -> List[Data]:
    ...
    if params.feature_type == "atomic_number":
        ...
        with open("dictionary_default.json", "r") as f:
    ...
    for struct in ase_db.select():
        ...
        if params.feature_type == "atomic_number":
            atom_features = np.vstack(
                [atomic_number_dict[i] for i in atom_obj.get_atomic_numbers()]
            )
        ...
        data = Data(
            ...
            edge_weight=torch.tensor(edge_weight, dtype=dtype),
            edge_attr=torch.tensor(edge_weight, dtype=dtype),
        )
